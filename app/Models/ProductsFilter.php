...
class ProductsFilter extends Model
{
    ...
    // Get the Publisher of a product from a URL (URL of the category)
    public static function getPublishers($url) { // this method is used in filters.blade.php
        // Get the parent category & its subcategories (child categories) ids of a certain URL
        $categoryDetails = Category::categoryDetails($url);
        ...
        // Get the Publisher ids of the product ids of the fetched categories (depending on the URL)
        $publisherIds = Product::select('publisher_id')->whereIn('id', $getProductIds)->groupBy('publisher_id')->pluck('publisher_id')->toArray(); // We used groupBy() method to eliminate the repeated product `publisher_id`-s (in order not to show repeated 'filters' values (like Samsung, Samsung, LG, ...)): https://laravel.com/docs/9.x/collections#method-groupby


        // Get the Publisher names from `publishers` table
        $publisherDetails = \App\Models\Publisher::select('id', 'name')->whereIn('id', $publisherIds)->get()->toArray(); // from `Publishers` table
