...
<script> // Note: We must use a <script> tag to write JavaScript because this file has a .php extension
    ...
    $(document).ready(function() {
        ...
        $('#sort').on('change', function() { // selecting the <selec> box in listing.blade.php
            ...
            var publisher = get_filter('publisher'); // get all the ':checked' checkboxes (the 'publisher' filter values) in filters.blade.php    // get the filter values array of 'publisher' filter like    ['Concrete', 'Adidas', ...]    as an ARRAY    // get_filter() is in front/js/custom.js
            ...
            $.ajax({
                ...
                data   : { // we pass the 'sort' (Sorting Filter), 'url' variables along with the all Dynamic Filters's values DYNAMICALLY
                    ...
                    @endforeach
                    sort: sort, url: url, size: size, color: color, price: price, publisher: publisher
                ...
                $.ajax({
                    ...
                    data   : {
                        ...
                        @foreach ($productFilters as $filters) // A new separate loop to send all the other remaining filters' values in the AJAX call, along with sending the current jQuery selected filter's values    // We have to loop over the main filters here AGAIN, otherwise the $(.filter) selector would select the filter values of ONE filter ONLY, and would ignore the filter values of all the other filters e.g. Without the foreach loop, it would select the 'color' filter values like: ['red', 'green'] but would ignore another filter like 'sleeve' filter and ignore its checked values like: ['full sleeve', 'half sleeve'] . Tip: Remove the foreach loop and change $filters to $filter and check the console (Don't forget to console.log(filter) inside the get_filter() function in front/js/custom.js)
                            {{ $filters['filter_column'] }}: {{ $filters['filter_column'] }}, // Note that fabric is an ARRAY of the filter values (like    ['cotton', 'polyester', ...]    ) of the 'fabric' filter    // send the Sorting Filters values (sort) along with the Dynamic Filters values ('fabric' Dynamic Filter values)
                        @endforeach
                        url: url, sort: sort, size: size, color: color, price: price, publisher: publisher
        ...
        // Size, price, color, publisher, … are also Dynamic Filters, but won't be managed like the other Dynamic Filters, but we will manage every filter of them from the suitable respective database table, like the 'size' Filter from the `products_attributes` database table, 'color' Filter and `price` Filter from `products` table, 'publisher' Filter from `publishers` table
        // Fourth: the 'publisher' filter (from `products` and `publishers` database tables)
        // WHEN the 'publisher' FILTER'S FILTER VALUE IS CLICKED, SEND THE CLICKED 'publisher' FILTER'S FILTER VALUES ALONG WITH THE OTHER FILTERS' FILTER VALUES TOO!!
        $('.publisher').on('click', function() { // select the 'publisher' filter in filters.blade.php
            var url  = $('#url').val(); // from the <select> box in listing.blade.php page (which, in turn, includes filters.blade.php page)
            var sort = $('#sort option:selected').val(); // select the :selected <option> element ONLY which is :selected in listing.blade.php (which, in turn, includes filters.blade.php) (like 'publisher_highest', 'name_z_a', ...)    // https://www.w3schools.com/jquery/sel_input_selected.asp    // .text() https://www.w3schools.com/jquery/html_text.asp    // send the Sorting Filters values (sort) along with the Dynamic Filters values ('fabric' Dynamic Filter values)
            ...
            // WHEN the 'publisher' FILTER'S FILTER VALUE IS CLICKED, SEND THE CLICKED 'publisher' FILTER'S FILTER VALUES ALONG WITH THE OTHER FILTERS' FILTER VALUES TOO!!
            @foreach ($productFilters as $filters) // A new separate loop to get all the other remaining filters' values, along with the current jQuery selected 'publisher' filter    // We have to loop over the main filters here AGAIN, otherwise the $(.publisher) selector would select the 'publisher' filter values ONLY, and would ignore the filter values of all the other filters e.g. Without the foreach loop, it would select the 'publisher' filter values like: ['Concrete', 'Adidas'] but would ignore another filter like 'sleeve' filter and ignore its checked values like: ['full sleeve', 'half sleeve'] . Tip: Remove the foreach loop and change $filters to $filter and check the console (Don't forget to console.log(filter) inside the get_filter() function in front/js/custom.js)
                var {{ $filters['filter_column'] }} = get_filter('{{ $filters['filter_column'] }}'); // get all the ':checked' checkboxes (the 'fabric' filter values) in filters.blade.php    // get the filter values array of 'fabric' filter like    ['cotton', 'polyester', ...]    as an ARRAY    // get_filter() is in front/js/custom.js
            @endforeach
            ...
            $.ajax({
                ...
                data   : {
                    ...
                    // WHEN the 'publisher' FILTER'S FILTER VALUE IS CLICKED, SEND THE CLICKED 'publisher' FILTER'S FILTER VALUES ALONG WITH THE OTHER FILTERS' FILTER VALUES TOO!! (Tip: Remove the foreach loop and change $filters to $filter and check the console (Don't forget to console.log(filter) inside the get_filter() function in front/js/custom.js))
                    @foreach ($productFilters as $filters) // A new separate loop to send all the other remaining filters' values in the AJAX call, along with sending the current jQuery selected 'publisher' filter's values    // We have to loop over the main filters here AGAIN, otherwise the $(.publisher) selector would select the 'publisher' filter values ONLY, and would ignore the filter values of all the other filters e.g. Without the foreach loop, it would select the 'publisher' filter values like: ['Concrete', 'Adidas'] but would ignore another filter like 'sleeve' filter and ignore its checked values like: ['full sleeve', 'half sleeve'] . Tip: Remove the foreach loop and change $filters to $filter and check the console (Don't forget to console.log(filter) inside the get_filter() function in front/js/custom.js)
                        {{ $filters['filter_column'] }}: {{ $filters['filter_column'] }}, // Note that fabric is an ARRAY of the filter values (like    ['cotton', 'polyester', ...]    ) of the 'fabric' filter    // send the Sorting Filters values (sort) along with the Dynamic Filters values ('fabric' Dynamic Filter values)
                    @endforeach
                    url: url, sort: sort, size: size, color: color, price: price, publisher: publisher
